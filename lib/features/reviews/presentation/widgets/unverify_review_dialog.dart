...
class UnverifyReviewDialog extends StatelessWidget {
  final int id;
  final ReviewEntityData review;
  const UnverifyReviewDialog({super.key, required this.id, required this.review});
          ...
          children: [
            ...
            BlocConsumer<ReviewsBloc, ReviewsState>(
              ...
              listener: (context, state) {
                if (state.unverifyResponse.status == Status.error) {
                  context.flushBarErrorMessage(
                      message: state.unverifyResponse.message.toString());
                }
                if (state.unverifyResponse.status == Status.completed) {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: CustomButton(
                        ...
                        text: "Confirm",
                        isLoading:
                            state.unverifyResponse.status == Status.loading,
                        bgColor: Colors.red,
                      ),
                    ),
                  ],
