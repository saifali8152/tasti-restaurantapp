...
import 'package:tasti_restaurant_app/features/reservations_db/data/models/csv_data.dart';
import 'package:tasti_restaurant_app/features/reservations_db/data/models/resevation_data.dart';
import 'package:tasti_restaurant_app/features/reservations_db/data/models/resevation_data_email.dart';
...
class ReservationDbRemoteApiImpl extends IReservationDbRemoteApi {
  ...
  Future<String> importCSVFile(ImportCSVFileParms parms) async {
    ...
    var response = await networkApiService.postMultipart(AppUrls.importCSV, data, files);
  ...
  Future<List<CSVDataModel>> fetchCSVData(String id) async{
    final parms = {"id": id};
    var response = await networkApiService.get(AppUrls.getCsvData, queryParams: parms);

    final List<dynamic> rawList = response['data'];
    final List<CSVDataModel> data = rawList.map((val)=> CSVDataModel.fromJson(val)).toList();
    return data;
  }
  ...
  Future<List<ReservationDataModel>> fetchReservations(String id)async {
    final parms = {"id": id};
    var response = await networkApiService.get(AppUrls.getRestaurantDatabase, queryParams: parms);

    final List<dynamic> rawList = response['data'];
    final List<ReservationDataModel> data = rawList.map((val)=> ReservationDataModel.fromJson(val)).toList();
    return data;
  }
  
  @override
  Future<List<ReservationDataEmailModel>> fetchReservationsByEmail(FetchReservationByEmailParms parms) async {
    final queryParms = {"id": parms.restaurantId.toString(), "email": parms.email.toString()};
    var response = await networkApiService.get(AppUrls.getReservationsByEmail, queryParams: queryParms);

    final List<dynamic> rawList = response['data'];
    final List<ReservationDataEmailModel> data = rawList.map((val)=> ReservationDataEmailModel.fromJson(val)).toList();
    return data;
  }
}
